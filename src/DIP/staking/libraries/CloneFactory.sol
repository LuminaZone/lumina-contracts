...
library CloneFactory {
    ...
    function _createCloneWithSaltOnConstructor(
        address target,
        bytes32 salt,
        ...
        assembly {
            ...
            // Copy constructor arguments to clone
            let argsLength := mload(constructorArgs)
            mstore(add(clone, 0x4c), argsLength)
            mstore(add(clone, 0x74), constructorArgs)

            // Use create2 with the provided salt
            result := create2(0, clone, add(0x74, argsLength), salt)
            if iszero(extcodesize(result)) {
                revert(0, 0)
            }
        }
